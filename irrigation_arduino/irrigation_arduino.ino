...
void loop()
{
  if (Serial.available())
  {
    ...
    int numRecipients = countOccurrences(recipientData, ',') + 1;
    ...
    else if (intValue > 511 && !sentMessage)
    {
      ...
      for (int i = 0; i < numRecipients; i++)
      {
        ...
        delay(4000); // Adding a delay to ensure proper message transmission between recipients
      }
...
void SendMessage(String recipient)
{
 if (SIM900A.available()) {
  ...
  recipient.trim(); // Trim any leading or trailing whitespaces or newlines
